...
//	ib_dev = NULL;
	...
	rdma->s_ctx->gidIndex = ib_gid;
	
	for( int i = 0; i < connect_number; i ++ ){
		build_connection( rdma, i );
	}
...
void build_connection(struct rdma_management *rdma, int tid)
{
	...
	if( !tid ){
	  build_context(rdma->s_ctx);
	  register_memory( rdma->memgt, rdma->s_ctx, end );
...
void post_recv( struct rdma_management *rdma, int qp_id, ull tid, int offset, int recv_size)
{
	...
	sge.addr = (uintptr_t)rdma->memgt->recv_buffer+offset;
	sge.length = recv_size;
	sge.lkey = rdma->memgt->recv_mr->lkey;
	
	TEST_NZ(ibv_post_recv(rdma->qpmgt->qp[qp_id], &wr, &bad_wr));
}
...
void post_send( struct rdma_management *rdma, int qp_id, ull tid, int offset, int send_size, int imm_data )
{
	...
	sge.addr = (uintptr_t)rdma->memgt->send_buffer+offset;
...
int get_wc( struct rdma_management *rdma, struct ibv_wc *wc )
{
	...
	int ret;
	while(1){
		ret = ibv_poll_cq(rdma->s_ctx->cq_data[0], 1, wc);
		if( ret > 0 ) break;
	}
...
{
	for( int i = 0; i < connect_number; i ++ ){
		ibv_destroy_qp( rdma->qpmgt->qp[i] );
	}
	free(rdma->qpmgt); rdma->qpmgt = NULL;
	return 0;
}
...
{
	for( int i = 0; i < cq_data_num; i ++ )
		TEST_NZ(ibv_destroy_cq(rdma->s_ctx->cq_data[i]));
	for( int i = 0; i < cq_ctrl_num; i ++ )
		TEST_NZ(ibv_destroy_cq(rdma->s_ctx->cq_ctrl[i]));
	TEST_NZ(ibv_destroy_cq(rdma->s_ctx->cq_mem[0]));
	free(rdma->s_ctx->cq_data); rdma->s_ctx->cq_data = NULL;
...
int destroy_memory_management( struct rdma_management *rdma, int end )// 0 active 1 backup
{	
	TEST_NZ(ibv_dereg_mr(rdma->memgt->send_mr));
	free(rdma->memgt->send_buffer);  rdma->memgt->send_buffer = NULL;
	
	TEST_NZ(ibv_dereg_mr(rdma->memgt->recv_mr));
	free(rdma->memgt->recv_buffer);  rdma->memgt->recv_buffer = NULL;
	
	if( end == 0 ){//active
		TEST_NZ(ibv_dereg_mr(rdma->memgt->rdma_send_mr));
	}
	else{//backup
		TEST_NZ(ibv_dereg_mr(rdma->memgt->rdma_recv_mr));
		free(rdma->memgt->rdma_recv_region); rdma->memgt->rdma_recv_region = NULL;
	}
	
	
	free(rdma->memgt); rdma->memgt = NULL;
	return 0;
}
